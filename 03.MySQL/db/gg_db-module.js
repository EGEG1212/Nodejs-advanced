...
module.exports = {                          //module.exports
    ...
    getAllLists:    function (callback) {       //콜백들어감
        ...
        let sql = `SELECT ggid, NAME as singer, debut, hit_song_id  FROM girl_group  ORDER BY ggid LIMIT 10;`;
    ...
    insertGg:     function (params, callback) {
        let sql = `insert into girl_group (singer, debut) values (?, ?);`; //고수는 파라메타로 받는다!!!  ?. ? 하나하나 해당된다.
    ...
    deleteGg:     function (ggid, callback) {
        ...
        let conn = this.getConnection(); 
        conn.query(sql, ggid, function(error, fields) {        //1개삭제여서 sid만 넣음됨
            if (error)
                console.log(error);
            callback();             //여기는 콜백 mvc.js에는 27줄 dm.insertSong(params,
        });
    ...
    updateGg:     function (params, callback) {      //이 순서대로 3개모두 받아야해서 params
        let sql = `update girl_group  set singer=?, debut=?, where ggid=?;`; 
        let conn = this.getConnection(); 
