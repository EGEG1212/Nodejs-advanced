...
const dm = require('./db/mvcdb-module');           //이런 펑션을 쓸꺼야~
...
app.get('/', (req, res) => {
    ...
    dm.getJoinLists(rows => {
        const View = require('./view/mvcjoin') ;       //상단에 있다가 여기서 열어꺼내는걸로
        let html = View.mainForm(rows);
        res.end(html);
    });
});
...
app.get('/insert', (req, res) => {                //읽어서 화면을 클라이언트에게 던져주려
    const view = require('./view/mvcinsert');      //모듈에서 읽는형태. 앞으로 이렇게 할것
...
app.get('/delete/:sid', (req, res) => {         //😛삭제는 get만 해도되나?
...
app.get('/update/:sid', (req, res) => {         //😋업데이트는 1)data가져와서,form만들기 2)form보여주기
    let sid = parseInt(req.params.sid);         //요렇게 받아요
    dm.getSong(sid, result => {
        const View = require('./view/update') ;       //상단에 있다가 여기서 열어꺼내는걸로
...
app.post('/update', (req, res) => {
    ...
    dm.updateSong(params, () => {           //🎈파람스로받고 콜백할게 없어서 빈칸?
        res.redirect('/');
    })
});
